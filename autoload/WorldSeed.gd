...
func seed_if_empty(player_id: StringName) -> void:
	...
	var venue_id := PlaceRepo.create_place(&"venue", "Le Petit Club", {"pos": Vector2(200,40), "account_id":club_account})
	var home_id := PlaceRepo.create_place(&"home", "Appartement du Joueur", {"pos": Vector2(20,30)})
	var studio_id := PlaceRepo.create_place(&"studio", "Studio A", {"pos":Vector2(120,80)})
